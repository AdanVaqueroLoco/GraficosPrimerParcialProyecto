...
def actualizar():
    ...
    if estado_tecla_arriba == glfw.PRESS:
        if not colisionando(
            posicion_nave[0],posicion_nave[1] + cantidad_movimiento,
            0.05,0.05,0.05,0.05,
            posicion_limite1[0], posicion_limite1[1],
            1.00,1.00,0.7,0.7):
            posicion_nave[0] = posicion_nave[0] + (math.cos((angulo_nave + fase) * pi / 180.0) * cantidad_movimiento)
            posicion_nave[1] = posicion_nave[1] + (math.sin((angulo_nave + fase) * pi / 180.0) * cantidad_movimiento)
    ...
    if estado_tecla_derecha == glfw.PRESS:
        ...
        if not colisionando(
            posicion_nave[0] + cantidad_movimiento,posicion_nave[1],
            0.05,0.05,0.05,0.05,
            posicion_limite1[0], posicion_limite1[1],
            1.00,1.00,0.7,0.7):
            posicion_nave[0] = posicion_nave[0] 
...
def draw_limite1():
    global posicion_limite1
    glPushMatrix()
    glTranslatef(posicion_limite1[0], posicion_limite1[1], 0.0)
    glBegin(GL_QUADS)
    glColor3f(0.4, 0.9, 0.21)
    glVertex3f(-1.00,1.0,0.0)
    glVertex3f(1.00,1.0,0.0)
    glVertex3f(1.0,-0.7,0.0)
    glVertex3f(-1.00,-0.7,0.0)

...
##Enemigos
posiciones_enemigos = [-0.4, 0.75, 0.0]
velocidades_enemigos = [0.1]
#direcciones 0 abajo 1 arriba
direcciones_enemigos = [0]
activos_enemigos = [1]
    ...
    for i in range(3):
        if activos_enemigos:
            ...
            if direcciones_enemigos[i] == 0:
                posiciones_enemigos[0] = posiciones_enemigos[0] - cantidad_movimiento
                if posiciones_enemigos[0] <= -0.75:
                    direcciones_enemigos[i] = 1
            else:
                posiciones_enemigos[0] = posiciones_enemigos[0] + cantidad_movimiento
                posiciones_enemigos[1] = posiciones_enemigos[1] - (cantidad_movimiento/20)
                if posiciones_enemigos[0] >= 0.75:
    ...
    if estado_tecla_derecha == glfw.PRESS:
        for i in range(3):
         if not colisionando(
            ...
            0.05,0.05,0.05,0.05,
            posiciones_enemigos[0] , posiciones_enemigos[1] ,
            0.05,0.05,0.2,0.2):
...
def draw_enemigos(): ##verde
    ...
    for i in range(3):
        glPushMatrix()
        glTranslatef(posiciones_enemigos[0], posiciones_enemigos[1], 0.0)
        glBegin(GL_QUADS)

...
posiciones_cuadrados2 = [
    [-0.2, 0.75, 0.0], 
    [0.2, 0.75, 0.0], 
    [0.6, 0.75, 0.0]]
velocidades_cuadrados2 = [0.1,0.35,0.2]
#direcciones 0 abajo 1 arriba
direcciones_cuadrados2 = [0,0,0]
activos_cuadrados2 = [1,0,0]
...
def actualizar_cuadrado2(tiempo_delta):
    global contador_tiempo

    if glfw.get_time() > 3.0:
        activos_cuadrados2[2] = 1
    if glfw.get_time() > 7.0:
        activos_cuadrados2[1] = 1

    contador_tiempo = contador_tiempo + tiempo_delta
    if contador_tiempo >= 1.0:
        contador_tiempo = contador_tiempo - 1.0
        for i in range(3):
            velocidades_cuadrados2[i] = velocidades_cuadrados2[i] + 0.01
    for i in range(3):
        if activos_cuadrados2[i]:
            cantidad_movimiento = velocidades_cuadrados2[i] * tiempo_delta
            if direcciones_cuadrados2[i] == 0:
                posiciones_cuadrados2[i][1] = posiciones_cuadrados2[i][1] - cantidad_movimiento
                if posiciones_cuadrados2[i][1] <= -0.75:
                    direcciones_cuadrados2[i] = 1
            else:
                posiciones_cuadrados2[i][1] = posiciones_cuadrados2[i][1] + cantidad_movimiento
                if posiciones_cuadrados2[i][1] >= 0.75:
                    direcciones_cuadrados2[i] = 0
def actualizar():
    global tiempo_anterior
    global window
    global posicion_triangulo
    global posicion_cuadrado
    global posicion_cuadrado2
    ...
    actualizar_cuadrado2(tiempo_delta)
...
def draw_triangulo():
    ...
    for i in range(3):
        if colisionando(posicion_triangulo[0],posicion_triangulo[1],
            ...
            break
        elif colisionando(posicion_triangulo[0],posicion_triangulo[1],
            0.05,0.05,0.05,0.05,
            posiciones_cuadrados2[i][0], posiciones_cuadrados2[i][1],
            0.05,0.05,0.05,0.05):
            glColor3f(0,0,1)
            glfw.set_window_should_close(window,True)
            break
...
def draw_cuadrado2():
    global posiciones_cuadrados2
    for i in range(3):
        glPushMatrix()
        glTranslatef(posiciones_cuadrados2[i][0], posiciones_cuadrados2[i][1], 0.0)
        glBegin(GL_QUADS)
        glColor3f(0.4, 0.9, 0.21)
        glVertex3f(-0.05,0.05,0.0)
        glVertex3f(0.05,0.05,0.0)
